...
The R package `soilscaler` contains functions to downscale coarse-resolution soil maps to higher resolutions by combining high-resolution covariates with soil observations from other sites. The goal of the package is to provide a workflow which is user-friendly and straightforward, while being flexible enough to allow experimentation.
